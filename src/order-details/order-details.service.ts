...
export class OrderDetailsService {
  ...
  async create(createOrderDetailDto: CreateOrderDetailDto): Promise<ApiOneResponse<OrderDetailEntity>> {
    try {
      const order = await this.orderDetailsRepository.save(createOrderDetailDto);
      if (!order) {
  ...
  async findAll(paginationDto: PaginationDto): Promise<ApiAllResponse<OrderDetailEntity>> {
    ...
    try {
      ...
      const [total, data] = await Promise.all([
        ...
        this.orderDetailsRepository.createQueryBuilder("orderDetail")
  ...
  async findOne(id: string): Promise<ApiOneResponse<OrderDetailEntity>> {
    try {
      const order = await this.orderDetailsRepository.createQueryBuilder("orderDetail")
        .leftJoinAndSelect("orderDetail.order", "order")
        .leftJoinAndSelect("orderDetail.product", "product")
  ...
  async update(id: string, updateOrderDetailDto: UpdateOrderDetailDto): Promise<ApiOneResponse<UpdateResult>> {
    try {
      const order = await this.orderDetailsRepository.update( {id, isActive: true}, updateOrderDetailDto );
      if( order.affected === 0 ){
        throw new ManagerError({
          type: "NOT_FOUND",
          message: "Order details not found!",
  ...
  async remove(id: string): Promise<ApiOneResponse<UpdateResult>> {
    try {
      const order = await this.orderDetailsRepository.update( {id, isActive: true}, {isActive: false} );
      if( order.affected === 0 ){
        throw new ManagerError({
          type: "NOT_FOUND",
          message: "Order details not found!",
