...
export class DeliveryOrdersService {
  ...
  async create(createDeliveryOrderDto: CreateDeliveryOrderDto): Promise<ApiOneResponse<DeliveryOrderEntity>> {
    try {
          const delivery = await this.deliveryOrdersRepository.save(createDeliveryOrderDto);
          if (!delivery) {
  ...
  async findAll(paginationDto: PaginationDto): Promise<ApiAllResponse<DeliveryOrderEntity>> {
    ...
    try {
      ...
      const [total, data] = await Promise.all([
        ...
        this.deliveryOrdersRepository.createQueryBuilder("deliveryOrder")
  ...
  async findOne(id: string): Promise<ApiOneResponse<DeliveryOrderEntity>> {
    try {
      const delivery = await this.deliveryOrdersRepository.createQueryBuilder("deliveryOrder")
  ...
  async update(id: string, updateDeliveryOrderDto: UpdateDeliveryOrderDto): Promise<ApiOneResponse<UpdateResult>> {
    try {
      const delivery = await this.deliveryOrdersRepository.update({ id, isActive: true }, updateDeliveryOrderDto);
  ...
  async remove(id: string): Promise<ApiOneResponse<UpdateResult>> {
    try {
      const delivery = await this.deliveryOrdersRepository.update({ id, isActive: true }, { isActive: false });
      if (delivery.affected === 0) {
        throw new ManagerError({
          type: "NOT_FOUND",
          message: "Delivery order not found!",
