...
export class DiscountsService {
  ...
  async findAll(paginationDto: PaginationDto): Promise<ApiAllResponse<DiscountEntity>> {
    ...
    try {
      ...
      const [total, data] = await Promise.all([
        ...
        this.discountsRepository.createQueryBuilder("discount")
          .where({ isActive: true })
          .skip(skip)
          .limit(limit)
          .getMany(),
  ...
  async update(id: string, updateDiscountDto: UpdateDiscountDto): Promise<ApiOneResponse<UpdateResult>> {
    try {
      const discount = await this.discountsRepository.update({ id, isActive: true }, updateDiscountDto);
  ...
  async remove(id: string): Promise<ApiOneResponse<UpdateResult>> {
    try {
      const discount = await this.discountsRepository.update({ id, isActive: true }, { isActive: false });
      if (discount.affected === 0) {
        throw new ManagerError({
          type: "NOT_FOUND",
          message: "Discount not found!",
