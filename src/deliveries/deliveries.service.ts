...
export class DeliveriesService {
  ...
  async findAll(paginationDto: PaginationDto): Promise<ApiAllResponse<DeliveryEntity>> {
  ...
  async findOne(id: string): Promise<ApiOneResponse<DeliveryEntity>> {
    try {
      const delivery = await this.deliveriesRepository.createQueryBuilder("delivery")
        .leftJoinAndSelect("delivery.shipper", "shipper")
        .leftJoinAndSelect("delivery.deliveryOrder", "deliveryOrder")
  ...
  async update(id: string, updateDeliveryDto: UpdateDeliveryDto): Promise<ApiOneResponse<UpdateResult>> {
    try {
      const delivery = await this.deliveriesRepository.update({ id, isActive: true }, updateDeliveryDto);
  ...
  async remove(id: string): Promise<ApiOneResponse<UpdateResult>> {
    try {
      const delivery = await this.deliveriesRepository.update({ id, isActive: true }, { isActive: false });
      if (delivery.affected === 0) {
        throw new ManagerError({
          type: "NOT_FOUND",
          message: "Delivery not found!",
